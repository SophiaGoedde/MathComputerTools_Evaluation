  ...
  - name: S. Gödde
    affiliations:
      - name: AgroParisTech
        ...
        country: France
        postal-code: 34090
    email: sophia.goedde@agroparistech.fr
  - name: K. Faber
    affiliations:
      - name: AgroParisTech
        city: Montpellier
        country: France
        postal-code: 34090
    email: kate.faber@agroparistech.fr
abstract: |
  Lizards worldwide are threatened by habitat loss and climate change, making it important to identify which habitats species depend on and whether these preferences can be predicted from simple field measurements. Using a global dataset of 700 lizard populations, we tested whether snout–vent length (SVL) and latitude—two traits easily obtained by researchers and citizen scientists—can predict preferred habitat type with KNN and random forest models. Both algorithms performed poorly due to extreme class imbalance and the limited ecological information contained in these predictors, with most models defaulting to the dominant terrestrial class. We also examined whether habitat preference aligns with lizard families and whether family-level traits (latitudinal range, foraging mode, tropical distribution) explain these patterns. Correspondence analyses revealed some lineage–habitat associations, but family traits did not significantly predict habitat use. Overall, SVL and latitude alone are insufficient for predicting habitat type, and simple descriptors cannot fully capture ecological specialization. Improving trait coverage and balancing habitat representation will be essential for future conservation-focused habitat modelling.
keywords: [Lizard, citizen science, habitat  conservation, KNN, Random Forest, correspondence analysis]
...
As lizard populations face extinction pressures around the globe from climate and land-use change [@jiang2023; @cosendey2023], habitat protection is a primary strategy for lizard conservation. Therefore understanding which habitat types are associated with a given lizard is needed. 

Lizards show a wide ecological and morphological diversity, with species occupying a wide range of habitats [@pianka2003]. Differentiation across the landscape of resource availability, predation risk, and microhabitat complexity create different selection pressures for a species’ size and thermal constraints [@pianka2003]. Therefore, morphological traits, such as a lizard’s body size, measured as snout–vent length (SVL), are often related to specific habitat specializations [@muñoz2018].  SVL influences motion, thermal regulation, and microhabitat use [@muñoz2018; @meiri2008].  

Broad-scale environmental variation is captured by latitude [@buckley2010]. Additionally, if a converse Bergmann’s rule applies to lizards, where larger-bodied species occur at lower latitudes, then SVL should show predictable geographic trends. Several studies support this theory demonstrating a decrease in size with increasing latitude within genuses [@ashton2003; @oufiero2011; @liang2021].  However, other work has documented opposing trends [@olalla-tarraga2006].

Both SVL and latitude are simple field measurements to observe and could offer a practical way to predict further ecological traits. This is useful for species lacking detailed natural-history data [@lambert2013]. For many incidental encounters, SVL and coordinates may be the only consistent measurements available. The practicality of minimal measurements also extends to the growing contribution of citizen science. Photo-identification  platforms increasingly provide georeferenced observations, and some of which can estimate body size. Habitat protection and species-monitoring strategies have the potential to be informed by these basic descriptors through prediction of an individual’s preferred habitat. Thus, habitat associations even for sparsely documented taxa can be inferred and locations where additional individuals may be anticipated [@poe2018].

Conservation may be linked to a whole lizard family instead of a species. Understanding which families are linked to particular habitats, can help anticipate which evolutionary lineages are most vulnerable to land-use and climate change when these habitat types are threatened [@buckley2010]. Habitat-specific families may face a disproportionate risk if their preferred environments change, while generalist families may be more resilient. 

In the context of lizard habitat conservation using citizen science, we asked the following questions: (1) Can female SVL and latitude accurately predict a lizard’s preferred habitat type? (2) Do preferred habitats correspond to lizard families? It was hypothesised that (H1) preferred habitat can be predicted from SVL and latitude because of known trends such as the Bergmann’s rule and demonstrated morphological trait relationships to specialized habitats [@liang2021; @muñoz2018]. It was also hypothesised that (H2a)  habitats and lizard families are correlated since families often show similar tendencies due to phylogenetic similarities in behaviour and traits [@oshea2021]. It is further predicted that (H2b) a family’s distribution, geographic range, and predominant foraging mode can explain their relationship to preferred habitats. Foraging mode is closely tied to a populations’ diet and thus their habitat [@huey1981]. Range and distribution influence the habitat types available.
...
A global database of 737 lizard populations and their traits was used to explore lizard habitat relationships (Mesquita 2015). Each population included data of latitude, longitude, mean female SVL (numerical), distribution (tropical/temperate), foraging mode (active/sit and wait), preferred habitat type, family, and species. All analyses were conducted in R (Version 2025.09.1+401). 

## Predicting preferred habitat

To test whether morphological traits (SVL) and latitude can predict preferred habitat of lizard populations, KNN and random forest models were used and later compared. The aim was to find the best predictive model of a lizard's habitat as a function of SVL and latitude. SVL and latitude were selected because of their easy observation in the field and limited number of missing observations(observations used: 689). Additionally, preliminary correlation checks, exploratory plots, and PCA showed these variables were not strongly correlated, but also suggested weak associations between SVL, latitude, and habitat. Other predictors were not included due to many missing values and the higher difficulty to be collected by citizen scientists.. Habitat was converted to a factor and the data was randomly split into an 80–20 training–test set, stratified by habitat class.
...
# habitat ~ SVL
ggplot(sublizard, aes(x = Latitude, y = MeanFSVL, fill = Habitat_Type)) +
...
The KNN method was used because of its ability to handle non-linear relationships and a newly found lizard could be classified to a specific habitat type only by the majority vote of its nearest neighbours. The optimal number of neighbours (k) was selected through 10-fold cross-validation (caret package) with centering and scaling applied to the predictors.Performance of the predictions was assessed using a confusion matrix and prediction uncertainty grid. 
...
predicted_Habitat <- knn(train = select(sublizard,-Habitat_Type), 
test = grid_prediction, 
k = k_best, 
cl = sublizard$Habitat_Type, 
prob = TRUE) 
...
To improve habitat classification with SVL and latitude, a random forest model was applied using the same dataset and predictor variables because random forest models (RF) are also used for non-linear methods of supervised classification and better for dealing with unbalanced datasets.  Random forest hyperparameters were tuned using repeated 10-fold cross-validation (10 folds × 5 repeats) with a grid search over mtry = 1–2 , reflecting the two predictor variables. The optimal RF model (500 trees, mtry = 1) was then fit to the training set. 

Initial models of the dataset revealed that classification accuracy was strongly biased toward the dominant class (“terrestrial”). To evaluate and mitigate this class imbalance, three strategies were implemented.  Firstly, a class-weighted RF, where class weights were calculated as the inverse of habitat class frequency so rare categories received greater influence in the model. Secondly, a downsampled RF, where the majority class was reduced to match minority class sizes. Thirdly, the minority classes were upsampled to match the terrestrial majority class. Performance of these methods was compared using a confusion matrix.
...
# Accuracy
acc_base <- predict(forest_lizard_base, newdata = select(test_RF_lizard,- Habitat_Type)) %>% 
...
To compare the performance of KNN and baseline RF of the initial dataset, a final KNN model was fitted with the best k and evaluated on the same RF test set. Model performance for both methods was assessed using confusion matrices, overall accuracy, Cohen’s Kappa, and balanced accuracy.

## **Habitat type and family**

The dataset was transformed to an abundance matrix of preferred habitat and families once missing values were removed and only for families represented by more than 20 populations (n=12, total observations used: 635) . This filter was implemented so that low observation families did not dominate the correlations represented. For each family, range from minimum to maximum latitude, proportion of populations with active foraging mode, and proportion distributed in the tropics was calculated to describe family characteristics. Other quantitative trait variables were difficult to summarise and still be representative of full family diversity, thus were not used in this analysis. Proportion of active foraging is low when a ‘sit and wait’ strategy is more common, similarly low proportion of tropical distribution reflects high amounts of temperate distribution. Latitude range was used instead of minimum and maximum latitude (or absolute latitude) as it was theorized families with a larger range would have access to more habitat types, however this value does not represent even distribution of the family across this range. 
...
lizard2 = filter(lizard2, Genus != "")

# dataframe setup for ca &cca
aliz=lizard2 %>%
  group_by(Habitat_Type, Family) %>%
  summarise(n=n()) %>% 
  filter(Habitat_Type != "")
aliz
...
aliz %>% 
  filter( Family %in% Fam)%>%
  ggplot() +
  aes(x = Habitat_Type, y = Family, fill = n) + 
  geom_raster() + 
...
General correlations among and between habitats and families were observed by correspondence analysis. Canonical correspondence analysis (vegan package) was run with the variables of family characteristics to explain preferred habitats. Eigen values were used to determine the amount of inertia explained by this model and an ANOVA was used to test for the significance of model explanation and contribution of variables. 
...
#| warning: false
#| layout-ncol: 2
#| fig-cap: "Predicted habitat type (left) and prediction uncertainty (right) based on KNN classification."
...
ggplot(final_predictions) +
    ...
    geom_point(data = sublizard, aes(color = Habitat_Type), show.legend = FALSE) +
    labs(x = "Mean Female SVL", y = "Latitude",
    fill = "Uncertainty") +
    scale_x_continuous(expand = c(0, 0)) +
    scale_y_continuous(expand = c(0, 0)) +
...
Figure 2: Predicted habitat type (left) and prediction uncertainty (right) based on KNN classification.
...
#| message: false
#| warning: false
#| tbl-cap: "Comparison of KNN and Random Forest performance"
#| tbl-cap-location: top

## Comparison: KNN & RF
cm_knn_RF <- confusionMatrix(
  data      = best_knn,
  reference = test_set$Habitat_Type)

cm_rf_RF <- confusionMatrix(
  data      = rf_pred_base,
  reference = test_RF_lizard$Habitat_Type)
...
#| fig-height: 3
#| fig-width: 6
#| fig-cap: "Correspondence Analysis explained dimensions"

fviz_ca_biplot(result_CAsm, repel = TRUE,
  labelsize = 3,             
  repel.args = list(size = 2)) +
